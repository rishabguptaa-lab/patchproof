...
if (command === 'verify') {
  ...
  try{const root=path.resolve(target);const manifest=valueAfter(args,'--manifest')||'.patchproof/proofs.json';const bundle=await inspectProofBundle(root,manifest);console.log(bundle.sha256);}catch(error){console.error(`PatchProof could not hash proofs: ${error.message}`);process.exitCode=2;}
